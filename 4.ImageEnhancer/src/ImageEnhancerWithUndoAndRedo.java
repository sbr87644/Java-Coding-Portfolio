...
public class ImageEnhancerWithUndoAndRedo extends Component implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        gTemp.drawImage(biWorking, 0, 0, null);
        undoStack.push(temp);
        if (e.getSource() == exitItem) {
            System.exit(0);
        }
        if (e.getSource() != undoItem && e.getSource() != redoItem && !redoStack.isEmpty()) {
            redoStack.clear();
        }
        ...
        if (e.getSource() == undoItem) {
            gTemp.drawImage((BufferedImage) undoStack.pop(), 0, 0, null);
            redoStack.push(temp);
            gTemp = biFiltered.getGraphics();
            gTemp.drawImage((BufferedImage) undoStack.pop(), 0, 0, null);
        }
        if (e.getSource() == redoItem) {
            gTemp = biFiltered.getGraphics();
            gTemp.drawImage((BufferedImage) redoStack.pop(), 0, 0, null);
        }
        if (undoStack.getSize() > 0) {
            undoItem.setEnabled(true);
        } else if (undoStack.getSize() == 0) {
            undoItem.setEnabled(false);
        }
        if (redoStack.getSize() > 0) {
            redoItem.setEnabled(true);
        } else if (redoStack.getSize() == 0) {
            redoItem.setEnabled(false);
        }
