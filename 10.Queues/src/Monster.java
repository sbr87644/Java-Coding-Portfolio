...
public class Monster implements Comparable {
    ...
    public int compareTo(Object obj) {
        ...
        if(teethSize != temp.teethSize)
            return Integer.compare(teethSize, temp.teethSize);
        if(weight != temp.weight)
            return Integer.compare(weight, temp.weight);
        if(height != temp.height)
            return Integer.compare(height, temp.height);
        return 0;
    }
